...
#ifdef UNIX_SPELL
int
ask_spell_subs(char **replace, int *wcol, char *match)
{
	int rc = 0;
...
ask_subs()
{
	if (do_subs == 'y' || do_subs == 'n') {
		...
		case 2:
			do_subs =
			    askquest("Confirm substitution [ynaq]? ", "qyna");
			break;
...
void
make_fail(char *pattern, int m, int *fail)
{
	int i, j, k;
...
void
show_match(FILE *tfp, long newline, char *text, FILE *cfp, int spell)
{
	int i, tch, len;
	...
	oldc = ftell(cfp);
	if (spell)
		fseek(cfp, -1L, 1); /* Backup one character */
	tch = getc(cfp);
	if (hlcontext && tch != '\n' && tch != EOF) {
		putchar(tch);
		mcursor(&cl, &junk_above, text[i]);
...
int
do_substitute(char *pattern, char *replace)
{
	FILE *cfp;
	...
	m = strlen(pattern);
	fail =
	    (int *)malloc((m + 1) * sizeof(int)); /* oversize for m==1 case */
	...
	for (;;) {
		if (fch == EOF) {
			for (i = 0; i < k; i++) putc(match[i], tfp);
			fclose(cfp);
			if (nmatch == 0 && !spell)
			...
			free(match);
			return rc;
		} else if (fch == pattern[k] ||
		           (pattern[k] == CSEP && !isalpha(fch))) {
			match[k] = fch;
			if (++k == m) {
				/* Match found - all characters before the match
				 * have been written out and last character of
				 * match is in fch variable
				 */
				...
				if (do_subs == 'y' || do_subs == 'n') {
					...
					if (spell)
						show_match(tfp, newline, match,
						    cfp, 1);
					else
						show_match(tfp, newline,
						    replace, cfp, 0);
					putchar('\n');
				...
				if (do_subs == 'y' || do_subs == 'a') {
					...
					/* Continue Search by advancing text
					 * pointer */
					if (!spell)
						fch = getc(cfp);
					continue;
				} else
					/* Reject match - by pretending (fch !=
					 * pattern[k]) */
					k--;
			} else {
...
int
cliplast(char *lastbuf, int bufsize)
{
	FILE *cfp;
