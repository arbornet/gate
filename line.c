...
int
outcol(char *str, int n, int icol)
{
	int i, c = icol;
...
void
mcursor(int *col, int *lin, char ch)
{
	if (ch == '\t') {
		*col = tabcol(*col);
		if (*col > mycols) {
...
int
ggetline(char *buf, char *wbuf, int *wcol, char *prompt)
{
	...
	for (;;) {
		...
		if (ch == '\n') {
			...
			*wcol = 0;
			...
			if (linemode ||
			    (!backwrap || (rc = cliplast(bf, BUFSIZE)) == 1)) {
			...
			if (rc == 2) {
				printf("\n[sorry, previous line too long to "
				       "backwrap to]\n");
				bfi = 0;
				colo = pcol;
				lines_above = 0;
			...
			if (i == -1)
				flg = i =
				    bfi; /* no spaces -- break at last col */
			else
				flg = 0;
			*wcol = bfi - i;

			/* Backspace back and copy into next line buffer - no
			 * tabs here */
			while (bfi > i) {
				bs();
				putchar(' ');
