...
int
openfile(char *filename, int resume)
{
	int oumask;
	...
	if ((tfp = fopen(filename, "r+")) != NULL) {
		...
		if (resume || errno != ENOENT ||
		    (tfp = fopen(filename, "w+")) == NULL) {
			char buf[BUFSIZE];

			sprintf(buf, "%s: cannot %sopen %s", progname,
			    resume ? "re-" : "", filename);
			perror(buf);
			umask(oumask);
			return (1);
...
typetoend(int pcol)
{
	...
	int ch;
	int col =
	    0; /* keeping track of columns only matters if dont_tab is on */

	while ((ch = fgetc(tfp)) != EOF) {
		if (och == '\n') {
			for (col = 0; col < pcol; col++) putchar(' ');
			lines_above++;
		}
		och = ch;
		if (dont_tab) {
	...
	clearerr(tfp);
	if (lines_above > mylines - 1)
		lines_above = mylines - 1;
}
...
int
askquest(char *prompt, char *answers)
{
	int an, ch;
...
do_edit()
{
	...
	fclose(tfp);
	if ((ed = getenv("VISUAL")) == NULL && (ed = getenv("EDITOR")) == NULL)
		ed = DEFAULT_EDITOR;
