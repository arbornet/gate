...
int
sigact(int signum, void (*handler)(), struct sigaction *old)
{
	struct sigaction sa;
...
done(int rc)
{
	...
	if (rc == RET_ASKOK || didcheck) {
		while ((ch = askquest("Ok to enter this response? [ync?] ",
		            "nyc?h>")) == '?' &&
		       ch != 'h') {
...
#endif /*YAPP*/
		/* Make file readable to others before exiting.  Yapp always
		 * needs this, PicoSpan only needs it when changing bulletins
		 * and such.
		 */
		chmod(filename, 0644);
