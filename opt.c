...
void
parse_opts(char *string, char *source)
{
...
print_opts()
{
	...
	for (i = 0; opttab[i].name != NULL; i++) {
		...
		case OT_NOYES:
			sprintf(buf, "%s%s ",
			    (*(int *)opttab[i].store) ? "" : "no",
			    opttab[i].name);
			break;
		case OT_INTEGER:
			sprintf(buf, "%s=%d ", opttab[i].name,
			    *(int *)opttab[i].store);
			break;
		case OT_STRING:
			if (*firstin(opttab[i].store, " /t'\"") == '\0')
				sprintf(buf, "%s=%s ", opttab[i].name,
				    (char *)opttab[i].store);
			else if (strchr(opttab[i].store, '\042') == NULL)
				sprintf(buf, "%s=\042%s\042 ", opttab[i].name,
				    (char *)opttab[i].store);
			else
				sprintf(buf, "%s='%s' ", opttab[i].name,
				    (char *)opttab[i].store);
...
int
set_prompt(char *arg, char *source)
{
	char *c;
...
int
set_outdent(int val, char *source)
{
