...
void
read_file(char *rname, int strip)
{
	FILE *rfp;
	...
	while ((ch = fgetc(rfp)) != EOF) {
		if (!strip ||
			...
			printf(
			    "Stripping bad characters from file %s\n", rname);
		och = ch;
	}
	fclose(rfp);
...
copy_fp(FILE *wfp)
{
	...
	while ((ch = fgetc(tfp)) != EOF) putc(ch, wfp);
}
...
void
write_file(char *wname)
{
	FILE *fp;

	if (expand_tilde(wname))
		return;

	if ((fp = copy_file(wname)) == NULL)
		return;
	fclose(fp);
