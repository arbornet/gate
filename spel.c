...
void
spell_help(int guesses)
{
	printf("   Type a replacement for the misspelled word or:\n");
	if (guesses > 0) {
		printf(
		    "   <number>  to replace the word with guess number nn\n");
...
add_repl(char *word, int word_len, char *replace)
{
	...
	for (r = rephead; r != NULL; r = r->next) {
		if (!strncmp(word, r->old, word_len) &&
		    r->old[word_len] == '\0') {
			if (r->new == NULL && replace == NULL)
				return;

			if (r->new != NULL && replace != NULL &&
			    !strcmp(replace, r->new))
				return;
...
int
prev_repl(char *word, int word_len, char *replace)
{
	struct repl *r;
...
void
show_guesses(char **guesslist, int guesses)
{
	int cls, lns;
	int len, longest, i, j;
	...
	for (i = 0; i < lns; i++) {
		...
		for (j = lns; j + i < guesses; j += lns)
			printf("   %2d: %-*.*s", i + j + 1, longest, longest,
			    guesslist[i + j]);
		putchar('\n');
		junk_above++;
	}
...
void
show_line(char *line, int offset, int word_len)
{
	int cl = 1;
	int i;
...
void
do_replace(char *line, int offset, int *oldlen, char *replace)
{
...
int
set_language(char *str, char *source)
{
#ifdef ASPELL
	struct langl *l, *ll;
	char buf[BUFSIZE + 1];
	...
	for (l = langlist; l != NULL; l = l->next) printf("%s\n", l->lang);
	return 0;
#else
#ifdef DICT_DIR
	char buf[BUFSIZE + 1];
...
check_line(char *line)
{
	...
	while (fgets(buf, BUFSIZE, from) != NULL) switch (buf[0]) {
		case '\n': /* Line finished */
			return checkdone;
		...
		case '#':
			if (!checkdone) {
				parse_ispell(buf, &offset, &word_len, &guesses,
				    guesslist);
				offset += offset_adjust;
				offset_adjust -= word_len;
				checkdone = fix_word(line, offset, &word_len,
				    guesses, guesslist);
				offset_adjust += word_len;
			}
			break;
...
void
next_word(char *line, int *offset, int *word_len)
{
	int i;
	char *nw, *ew;
...
void
make_guesslist(char **guesslist, char *p, int *guesses)
{
	int off = -1, len = 0;
...
check_line(char *line)
{
	...
	for (;;) {
		...
		if (word_len == 1)
			continue; /* all one-letter words are correct */
		...
		checkdone =
		    fix_word(line, offset, &word_len, guesses, guesslist);
	}
	return checkdone;
}
#endif
...
parse_ispell(
    char *ispell, int *offset, int *word_len, int *guesses, char **guesslist)
{
	char *p;
...
int
fix_word(char *line, int offset, int *word_len, int guesses, char **guesslist)
{
	char st[BUFSIZE + 40];
	char bf[BUFSIZE];
	...
	if ((wcol = prev_repl(word, *word_len, bf)) < 0)
		return 0;
...
start_ispell(char *banner)
{
	...
	if ((ch = getc(from)) == EOF || ch != '@' ||
		...
		if (ch == '*')
			printf("  You might have ispell 4.0 (recompile with "
			       "GISPELL)\n");
		fclose(to);
		fclose(from);
		return 1;
...
set_language(char *str, char *source)
{
	...
	if (!strcmp(str, LANG_DEFAULT))
		return 1;

	for (i = 0; lang[i] != NULL; i++)
		if (!strcmp(str, lang[i]))
			return 1;
	...
	for (i = 0; lang[i] != NULL; i++) {
		printf("%s\n", lang[i]);
		if (f && !strcmp(lang[i], LANG_DEFAULT))
			f = 0;
	}
	if (f)
		printf("%s\n", LANG_DEFAULT);
...
#ifdef SIMPLE_SPELL
	sprintf(
	    buf, "%s/.dict%c", home, strcmp(language, "british") ? 'a' : 'b');
#else
	sprintf(
	    buf, "%s/.word%c", home, strcmp(language, "british") ? 'a' : 'b');
#endif
	...
	if (addfp == NULL) {
		...
		if (access(dict, R_OK)) {
			...
			printf("Creating %c%s English dictionary %s...\n",
			    islower(language[0]) ? toupper(language[0])
			                         : language[0],
			    language + 1, dict);
			printf("This will require about 50K of disk space.\n");
			if (askquest("Are you sure you want to do this? ",
			        "ny") == 'n')
				return;

			sprintf(buf, "%s %s%c > %s", SPELLIN_PATH, SYSDICT_PATH,
			    (strcmp(language, "british") ? 'a' : 'b'), dict);
		} else {
			/* Move our dictionary to tmp location */
			sprintf(tempfile, "%s.tmp", dict);
			if (mv_file(dict, tempfile))
				return;

			sprintf(
			    buf, "%s %s > %s", SPELLIN_PATH, tempfile, dict);
		}
...
cmd_spell(char *arg)
{
	...
	if ((sfp = pipe_thru(mkspellcmd())) == NULL)
		return;
